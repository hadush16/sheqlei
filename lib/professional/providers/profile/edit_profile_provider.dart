...
import 'package:sheqlee/professional/models/skill_model.dart';
...
class ProfileNotifier extends StateNotifier<ProfileState> {
  ...
  void removeLink(String platformName) {
    state = state.copyWith(
      socialLinks: state.socialLinks
          .where((link) => link['platform'] != platformName)
          .toList(),
    );
  }
  ...
  Future<bool> saveProfile() async {
    // 1. Simulate a 2-second network delay
    await Future.delayed(const Duration(seconds: 2));

    // 2. Here you would normally do your http.post
    // For now, we just return true to simulate success
    //print("Profile data saved locally: ${state.fullName}");
